      ...
      {!isOpen ? (
        ...
        <div className="bg-black/95 text-white p-4 rounded-xl shadow-2xl border border-red-500/50 w-64 space-y-4 backdrop-blur-sm animate-in fade-in slide-in-from-bottom-4">
          ...
          <div className="text-xs bg-white/5 p-2 rounded border border-white/10">
            ...
            <span className="font-mono text-yellow-400 font-bold ml-1">
              {currentRole || "N/A"}
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 gap-2">
            {["OWNER", "MANAGER", "MEMBER"].map((role) => (
              <Button
                ...
                onClick={() =>
                  switchRoleMutation.mutate({ targetRole: role as any })
                }
              >
                {role === currentRole && (
                  <div className="w-2 h-2 bg-white rounded-full mr-2 shadow-[0_0_8px_rgba(255,255,255,0.8)]" />
                )}
                Mudar para {role}
              </Button>
            ))}
